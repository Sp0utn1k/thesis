...
def generate_sample(vis_grid,max_length,max_id,max_ammo,device='cpu'):
    ...
    for _ in range(length):
        friend = generate_dummy_friend(vis_grid,idx=list(range(max_id+1)),ammo=list(range(max_ammo+1)))
        observation.append(friend)
        pos = friend[:2]
        if pos in positions:
            return generate_sample(vis_grid,max_length,max_id,max_ammo,device=device)
