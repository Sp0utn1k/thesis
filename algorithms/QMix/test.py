...
if __name__ == '__main__':
    ...
    print(network)
    qmix = QMix.QMix(env,network,unique_net=False,use_gpu=True,epsilon=1,batch_size=64,buffer_size=500,gamma=.99,lr=5e-4,
        mixer_hidden_layers=[16],net_sync_period=2,optimizer='rmsprop',use_mixer=True)
    ...
    obs1 = torch.tensor(obs1,device=qmix.device,dtype=torch.float32).repeat(4,1)
    obs2 = torch.tensor(obs2,device=qmix.device,dtype=torch.float32).repeat(4,1)
    state= torch.tensor(state,device=qmix.device,dtype=torch.float32).repeat(4,1)
